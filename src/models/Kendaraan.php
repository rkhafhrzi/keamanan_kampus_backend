...
class Kendaraan {
    ...
    public function semua($limit = 200) {
        $sql = "SELECT k.*, p.nama as pemilik FROM {$this->table} k LEFT JOIN pengguna p ON k.pengguna_id = p.id ORDER BY created_at DESC LIMIT :lim";
    ...
    public function buat(array $data) {
        $sql = "INSERT INTO {$this->table} (pengguna_id, plat_nomor, merek, warna, status) VALUES (:pengguna_id,:plat_nomor,:merek,:warna,:status)";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
    ...
    public function update($id, array $data) {
        $sql = "UPDATE {$this->table} SET plat_nomor=:plat_nomor,merek=:merek,warna=:warna,status=:status WHERE id = :id";
        $stmt = $this->db->prepare($sql);
        $data['id'] = $id;
        return $stmt->execute([
            ':plat_nomor'=>$data['plat_nomor'],
    ...
    public function findByPlat($plat) { return $this->cariByPlat($plat); }
}
